...
| `stopWhen: stepCountIs(8)` | README architecture diagram | `app/api/line/webhook/route.ts:719` | ❌ false | Code uses `stepCountIs(3)` for both Gemini and Groq. README says 8. |
...
| "Groq fallback: llama-4-scout → gpt-oss-120b" | CLAUDE.md | `lib/llm/provider.ts:43-49` | ⚠️ partial | Actual order is llama-4-maverick → llama-4-scout → gpt-oss-120b. CLAUDE.md doesn't list maverick. |
...
| "Inline stopWhen for Groq is also stepCountIs(3)" | inferred | `app/api/line/webhook/route.ts:820` | ✅ matches | Both Gemini and Groq use `stepCountIs(3)`. |
