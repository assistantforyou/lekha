...
| LLM | Vercel AI SDK v6 + `@ai-sdk/google` (Gemini Flash Lite primary) |
...
- **Gemini timeout at 60s** — Gemini occasionally hangs for 45+ seconds on overloaded requests. 60s gives healthy agentic turns room under latency spikes while still surfacing real hangs.
