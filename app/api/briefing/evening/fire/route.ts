...
export async function POST(req: NextRequest) {
  ...
  if (!allowManual) {
    ...
    try {
      const ok = await receiver.verify({
        signature: sig,
        body: raw,
        url: `${env().APP_BASE_URL}/api/briefing/evening/fire`,
      });
  ...
  if (!summary) {
    console.log("[briefing/evening] nothing to send", { userId });
  ...
  console.log("[briefing/evening] sent", { userId, timezone: settings.timezone });
