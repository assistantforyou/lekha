...
function inNoTranslate(node: Node): boolean {
...
function translateText(node: Text, toThai: boolean) {
...
function walk(start: Node, toThai: boolean) {
  if (start.nodeType === 3) {
    if (!inNoTranslate(start)) translateText(start as Text, toThai);
    return;
  }
  ...
  const walker = document.createTreeWalker(start, NodeFilter.SHOW_TEXT, {
    acceptNode: (n) =>
      inNoTranslate(n) ? NodeFilter.FILTER_REJECT : NodeFilter.FILTER_ACCEPT,
  });
  const nodes: Text[] = [];
  let n: Node | null;
    ...
    for (const m of muts) {
      if (m.type === "childList") {
        m.addedNodes.forEach((nd) => {
          if ((nd.nodeType === 1 || nd.nodeType === 3) && !inNoTranslate(nd))
            todo.push(nd);
        });
...
function setLang(thai: boolean) {
  isThai = thai;
  document.body.classList.toggle("lang-th", thai);
  if (observer) observer.disconnect();
  if (!root) root = document.getElementById("__next") ?? document.getElementById("root");
...
if (typeof document !== "undefined") {
  const attach = () => {
    root = document.getElementById("__next") ?? document.getElementById("root");
    startObserver();
  };
...
export function LangToggle() {
  ...
  const choose = (t: boolean) => { setThai(t); setLang(t); };
  return (
