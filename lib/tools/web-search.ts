...
export function buildWebSearchTool() {
  return {
    web_search: tool({
      description:
        "ALWAYS call this tool first when the user asks about current events, recent facts, live data, research questions, or anything that could have changed in the last few months. Never answer those questions from training data. Prefer news_search for breaking news/current events, crypto_price/stock_price for finance, and weather for forecasts — but web_search is always acceptable as a fallback if those tools fail or when the query is ambiguous. Pass count when the user requests a specific number of results (e.g. 'top 10').",
