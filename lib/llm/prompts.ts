...
- web_search — general search, macroeconomic data (GDP, CPI, employment, interest rates, economic indicators), AND explanatory questions about recent events ("why did X happen", "what caused X", "why did the market/sector/stock drop/rise"). NEVER for stock prices/crypto prices/FX rates/weather — use dedicated tools for those. For "why did X happen recently" questions, ALWAYS search — never answer from memory/training data.
