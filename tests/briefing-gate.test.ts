...
describe("shouldFireBriefingNow (Bangkok UTC+7)", () => {
  it("fires when local time is within the 15-min window and never fired before", () => {
    vi.useFakeTimers();
  ...
  it("does not fire at 07:15 local (after window)", () => {
    vi.useFakeTimers();
    freezeUtc(0, 15); // 07:15 BKK
    expect(shouldFireBriefingNow("07:00", null, "Asia/Bangkok")).toBe(false);
  });
...
describe("shouldFireEveningSummaryNow (Bangkok UTC+7, 9 PM target)", () => {
  it("fires when local time is 21:00–21:14 and never fired before", () => {
    vi.useFakeTimers();
  ...
  it("does not fire at 21:15 local (after window)", () => {
    vi.useFakeTimers();
    freezeUtc(14, 15); // 21:15 BKK
    expect(shouldFireEveningSummaryNow(null, "Asia/Bangkok")).toBe(false);
  });
